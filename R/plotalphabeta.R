...
#' @param d Effect size (Cohen's d): H1 mean is \code{m + d * sd0}.
...
#' @param show_text Logical. If TRUE, prints alpha, beta, and power in the plot.
...
plotalphabeta <- function(m = 0,
  ...
  if (fill) {
    df_fill <- rbind(
      data.frame(x = df$x[df$x >= crit],
                 ...
                 region = "Type I error (alpha)"),
      data.frame(x = df$x[df$x <= crit],
                 y = df$density_H1[df$x <= crit],
                 region = "Type II error (beta)")
    )
  }
  ...
  p <- ggplot(df, aes(x = x)) +
    ...
    labs(x = "x", y = "Density") +
    ggplot2::geom_line(aes(y = density_H0), color = "black", size = 0.7) +
    ggplot2::geom_line(aes(y = density_H1), color = "black", size = 0.7, linetype = "dashed") +
    ggplot2::geom_vline(xintercept = m, linetype = "dotted") +
    ggplot2::geom_vline(xintercept = m1, linetype = "dotted") +
  ...
  if (fill) {
    p <- p +
      ggplot2::geom_area(data = df_fill,
                aes(y = y, fill = region),
                alpha = 0.5,
                position = "identity",
                show.legend = TRUE) +
      ggplot2::scale_fill_manual(values = setNames(col_fill, c("Type I error (alpha)", "Type II error (Beta)")),
                        name = NULL)
  }
  
  # Textbox oben rechts
  if (show_text) {
    label <- sprintf("Alpha = %.3f\nBeta = %.3f\nPower = %.3f", alpha, beta, power)
  ...
  if (show_labels) {
    ...
    # H0 and H1 labels
    p <- p +
      ggplot2::annotate("text", x = m, y = ymax * 0.85, label = "H0", hjust = 1.2) +
      ggplot2::annotate("text", x = m1, y = ymax * 0.85, label = "H1", hjust = -0.2)
    # Effect size arrow
    p <- p +
      ggplot2::annotate("segment", x = m, xend = m1, y = ymax * 0.75, yend = ymax * 0.75,
               arrow = ggplot2::arrow(ends = "both", length = unit(0.1, "inches")),
               color = "darkgray") +
      ggplot2::annotate("text", x = (m + m1) / 2, y = ymax * 0.78, label = "Effect size (d)", hjust = 0.5)
  }
